...
def eliminar_producto(id: int, db: Session = Depends(get_db)):
    ...
    # Verificar si tiene detalles de venta registrados
    tiene_detalles = db.query(DetalleVenta).filter(DetalleVenta.id_producto == id).first()
    if tiene_detalles:
        raise HTTPException(
            status_code=409,
            detail="No se puede eliminar el producto porque tiene ventas registradas"
        )
